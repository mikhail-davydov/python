...
def check_list_sum(lst: list) -> int:
    filtered_lst = sorted(filter(lambda x: isinstance(x, int), lst))
    return max(filtered_lst, default=0) * min(filtered_lst, default=0)


def check_dict_sum(dct: dict) -> int:
    filtered_keys = filter(lambda key: isinstance(key, int), dct)
    return sum(filtered_keys)


def check_control_sum(filename: str, check_sum: int) -> str:
    with open(filename, 'rb') as i_file:
        obj = pickle.load(i_file)
        calculated_check_sum = check_list_sum(obj) if isinstance(obj, list) \
            else check_dict_sum(obj) if isinstance(obj, dict) \
            else 0
        return 'Контрольные суммы совпадают' if calculated_check_sum == check_sum else 'Контрольные суммы не совпадают'


print(check_control_sum(input().strip(), int(input())))
