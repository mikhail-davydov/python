...
group_by_name = lambda student: student.name
groups = groupby(sorted(students, key=group_by_name), key=group_by_name)
max_name = max(groups, key=lambda x: sum(1 for _ in x[1]))[0]
print(max_name)
